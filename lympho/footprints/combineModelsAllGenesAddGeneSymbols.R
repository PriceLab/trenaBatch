...
for(gene.dir in all.gene.directories[1:max]){
    ...
    if(file.exists(path.footprints)){
        ...
        if(nrow(tbl) == 0) next
        #print(gene.dir)
        if(gene.dir == "ENSG00000005001-PRSS22")
...
# some simple QC
tbl <- subset(tbl.models, rank <= 10)
dim(tbl)  # [1] 168235     13
tbl <- subset(tbl.models, rank <= 5)
dim(tbl) #  [1]  84239    13

tbl.tfDist <- as.data.frame(table(tbl$tf.symbol))
colnames(tbl.tfDist) <- c("tf", "count")
tbl.tfDist <- tbl.tfDist[order(tbl.tfDist$count, decreasing=TRUE),]
head(tbl.tfDist)
fivenum(tbl.tfDist$count)

hist(tbl.tfDist$count)

hist(tbl$pearsonCoeff)
fivenum(tbl$pearsonCoeff)
