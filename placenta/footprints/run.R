...
runGeneModel <- function(project, gene)
{
  ...
  if(nrow(tbl.enhancers) > 0){ 
   ...
   recipe <- list(title="PSG1 2kb only",
                  ...
                  databases=list("extraembryonic_structure_hint_20"),
   ...
   builder <- FootprintDatabaseModelBuilder("hg38", gene, recipe, quiet=FALSE)
...
if(!interactive()){
   ...
   #lapply(head(goi), function(gene) runGeneModel(project, gene))
